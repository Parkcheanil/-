		...
		<div class="sales_table">
			<form action="salesPerformenceBoard">
				<div class="search_area">
					<div class="listOrder">
						<select class="form-control search-select" style="width: 75px;"
					...
					<div class="searchProduct">
						<select class="form-control search-select" name="searchType">
							<option value="salespid"
								${pageVO.cri.searchType eq 'salespid' ? 'selected' : '' }>상품코드</option>
							<option value="salespname"
								${pageVO.cri.searchType eq 'salespname' ? 'selected' : '' }>상품명</option>
			...
			<div class="text-center">
				<ul class="pagination justify-content-center">
					<c:if test="${pageVO.prev }">
						<li class="page-item"><a href="#" class="pageLink"
							data-pagenum="${pageVO.startPage-1}">&laquo;</a></li>
					</c:if>

					<c:forEach var="num" begin="${pageVO.startPage }" end="${pageVO.endPage}">
						<li class="page-item ${num eq pageVO.pageNum ? 'active' : '' }">
							<a href="#" class="pageLink" data-pagenum="${num }">${num }</a>
						</li>
					</c:forEach>
...
<script>
	function handleChange(data) {
		...
		location.href = "salesPerformenceBoard?searchName=" + searchN + "&"
				+ "searchType=" + searchT + "&pageNum=1&amount=" + data.value;
	}
					...
					success : function(datas, status, xhr) { //응답 성공시
						...
						function drawChart() {
							...
							$.each(values, function(index, value) {
								console.log(index + " : " + value.salesdate
										+ " / " + value.salescount + " / "
										+ value.salesprice);

								itemsArr.push([ value.salesdate,
										value.salesprice ]);// 아이템 이름과 수량 추가
								itemsHistory.push([ value.salesdate,
										value.salesprice ]); //지속적으로 데이터 기록하기 위한 용도 - 한 번만 초기화

								let data = google.visualization
										.arrayToDataTable(itemsArr); // 그래프 데이터
								//console.log(data);
